...
public class UserLocalServiceImpl extends UserLocalServiceBaseImpl {
	...
	public boolean addDefaultGroups(User user) throws PortalException {
		...
		boolean addedDefaultRoleOrTeam = false;
		...
		for (String defaultGroupName : defaultGroupNames) {
			...
			if (group != null) {
				...
				else if (_addDefaultRolesAndTeams(
							group.getGroupId(),
							new long[] {user.getUserId()})) {

					addedDefaultRoleOrTeam = true;
				}
			}
		}
		...
		if (!groupIdsSet.isEmpty()) {
			long[] groupIds = ArrayUtil.toArray(
				groupIdsSet.toArray(new Long[0]));
		...
		else if (addedDefaultRoleOrTeam) {
			return true;
		}
