...
/**
 * Configuration listener that runs the registered {@link FipsValidator} for
 * the saved PID and rejects (Strict Mode) or warns (Standard Mode) on a
 * non-compliant {@link FipsReport}.
 *
 * <p>
 * The listener delegates the "is this PID FIPS-relevant?" question to the
 * presence of a registered validator: if no {@link FipsValidator} is
 * tracked for the PID being saved, the listener is a no-op. Provider modules
 * (or any module that wants its configuration FIPS-gated) register a
 * {@link FipsValidator} for the PID they want validated, regardless of
 * whether the PID belongs to the keymanager namespace.
 * </p>
 *
