...
public class ExportConfigurationMVCResourceCommandTest {
	...
	@Test
	public void testStringArrayWithLocationVariableResource() {
		Dictionary<String, Object> confProps = new Hashtable<String, Object>() {
			{
				put(
					"match-control",
					new String[] {"${resource://foo.bar/foo.bar}"});
				put(
					"match-variation1",
					new String[] {"${resource:/foo.bar/foo.bar}"});
				put(
					"match-variation2",
					new String[] {"${resource:foo.bar/foo.bar}"});
				put("match-variation3", new String[] {"${resource:}"});
				put(
					"mismatch-start",
					new String[] {"x${resource://foo.bar/foo.bar}"});
				put(
					"mismatch-end",
					new String[] {"${resource://foo.bar/foo.bar}x"});
				put(
					"mismatch-protocol",
					new String[] {"${xresource://foo.bar/foo.bar}"});
				put(
					"mismatch-separator",
					new String[] {"${resource+//foo.bar/foo.bar}"});
			}
	...
	@Test
	public void testStringWithLocationVariableResource() {
		Dictionary<String, Object> confProps = new Hashtable<String, Object>() {
			{
				put("match-control", "${resource://foo.bar/foo.bar}");
				put("match-variation1", "${resource:/foo.bar/foo.bar}");
				put("match-variation2", "${resource:foo.bar/foo.bar}");
				put("match-variation3", "${resource:}");
				put("mismatch-start", "x${resource://foo.bar/foo.bar}");
				put("mismatch-end", "${resource://foo.bar/foo.bar}x");
				put("mismatch-protocol", "${xresource://foo.bar/foo.bar}");
				put("mismatch-separator", "${resource+//foo.bar/foo.bar}");
			}
		};
	...
	protected void doAssertExportedConfigurationProperty(
		...
		String msg = "Exported property: " + key;
	...
	private ExtendedAttributeDefinition[] _mockExtendedAttributeDefinitions(
		Dictionary<String, Object> configurationProperties) {
	...
	private void _setUpConfigurationModelRetrieverMocks(
		...
		Mockito.doAnswer(
			(Answer<ExtendedAttributeDefinition[]>)
				invocationOnMock -> _mockExtendedAttributeDefinitions(
					configurationProperties)
		).when(
