...
test('Verify SSO login and logout mechanism works the same when having multiple sites configured as SP.  See LPS-170940.', async ({
	...
	const applicationsMenuPage = new ApplicationsMenuPage(sp2AdminPage);

	await applicationsMenuPage.goToSites(false);

	const sitesPage = new SitesPage(sp2AdminPage);

	await sitesPage.createSiteFromLiferayTemplate('Welcome', site1Name);
	...
	await sitesPage.createSiteFromLiferayTemplate('Welcome', site2Name);
	...
	// Delete newly created virtual instances
...
test('Verify the SAML configuration is not applied to the sites when ACS is disabled.  See LPS-170940.', async ({
	...
	const applicationsMenuPage = new ApplicationsMenuPage(spAdminPage);

	await applicationsMenuPage.goToSites(false);

	const sitesPage = new SitesPage(spAdminPage);

	await sitesPage.createSiteFromLiferayTemplate('Welcome', siteName);

	await spAdminPage.goto(`/web/${siteName}`);
